...
class CompositeProbabilities:
    ...
    def p_recursive(self, a, d):
        ...
        if a == 0:
            ...
            return self.basic_probs.p_swin(a, d) * self.p_recursive(a, d - 1) + (1 - self.basic_probs.p_swin(a, d)) * self.p_recursive(a - 1, d)
        ...
        for i in range(a + 1):
            for j in range(d + 1):
                if j == 0:
                    ...
                    y = self.basic_probs.p_swin(i, j) * risk_matrix[i][j - 1] + (1 - self.basic_probs.p_swin(i, j)) * risk_matrix[i - 1][j]
                risk_matrix[i][j] = y
        ...
        for i in range(a + 1):
            for j in range(d + 1):
                if j == 0:
                    ...
                    y = self.basic_probs.p_swin(i, j) * loss_matrix[i][j - 1] + (1 - self.basic_probs.p_swin(i, j)) * loss_matrix[i - 1][j]
                loss_matrix[i][j] = y
    ...
    def p_safe_stop(self, a, d):
        ...
        for d_losses in range(d-1):  # looping over all possible losses except for d-1 (when only 1 defender remains)
            ...
            p = (1-self.basic_probs.single_probs["3v2"]) * self.basic_probs.single_probs["3v2"]**(d-d_left) * (1-self.basic_probs.single_probs["3v2"])**(a-3) * factorial(d-d_left + a-3)/factorial(d-d_left)/factorial(a-3)
            probs.append(p)
        ...
        for i in range(3, a+1):  # columns
            for j in range(1, d+1):  # rows
                if j == 1:
                    ...
                    matrix[i][j] = matrix[i][j-1] * self.basic_probs.single_probs["3v2"] + matrix[i-1][j] * (1-self.basic_probs.single_probs["3v2"])
    ...
    def all_in(self, a, d, pick_result=True):
        ...
        for i in range(a):
            win_verluste[i] = i
            win_probs[i] = self.p_precise_win(a, d, a - i)

        # attacker loses
        for i in range(d):
            loss_verluste[i] = i
            loss_probs[i] = self.p_precise_loss(a, d, d - i)
    ...
    def safe_attack(self, a, d, pick_result=True):
        ...
        # attacker retreats
        retreat_probs = self.p_safe_stop(a, d)
