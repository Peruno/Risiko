...
def safe_attack(a, d, pick_result=True):
    ...
    win_probs = {i: probs.p_precise_win(a, d, a - i) for i in range(a - 2)}
    loss_probs = {i: j for (i, j) in zip(arange(d)[::-1], probs.p_safe_stop(a, d)[::-1])}
    ...
    if pick_result:
        scenario, troop_number = determine_result(win_probs, loss_probs, is_safe_attack=True)
        print_results(scenario, troop_number)
