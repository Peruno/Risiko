  ...
  group('Simulator', () {
    ...
    group('All-In Battle Simulation', () {
      ...
      test('allIn simulates outcomes correctly', () {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          expect([BattleOutcome.victory, BattleOutcome.defeat].contains(result.outcome), isTrue);
    ...
    group('Safe Attack Simulation', () {
      test('safeAttack input validation', () {
        expect(() => simulator.safeAttack(2, 5), throwsA(isA<ArgumentError>()));
        expect(() => simulator.safeAttack(5, 0), throwsA(isA<ArgumentError>()));
        expect(() => simulator.safeAttack(0, 3), throwsA(isA<ArgumentError>()));
      });
      ...
      test('safeAttack simulates outcomes correctly', () {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          expect([BattleOutcome.victory, BattleOutcome.retreat].contains(result.outcome), isTrue);
          expect(result.losses, greaterThanOrEqualTo(0));
        }
      ...
      test('Battle outcomes are consistent with losses', () {
        for (int i = 0; i < 50; i++) {
          ...
          if (allInResult.outcome == BattleOutcome.victory) {
            expect(allInResult.winProbabilities.keys.contains(allInResult.losses), isTrue);
          } else if (allInResult.outcome == BattleOutcome.defeat) {
            expect(allInResult.lossProbabilities.keys.contains(allInResult.losses), isTrue);
          }

          final safeResult = simulator.safeAttack(5, 3, simulateOutcome: true);

          if (safeResult.outcome == BattleOutcome.victory) {
            expect(safeResult.winProbabilities.keys.contains(safeResult.losses), isTrue);
          } else if (safeResult.outcome == BattleOutcome.retreat) {
            expect(safeResult.lossProbabilities.keys.contains(safeResult.losses), isTrue);
          }
        }
      });
    });
    ...
    group('Integration with Probability Calculations', () {
      ...
      test('probability distributions are mathematically sound', () {
        ...
        for (final testCase in testCases) {
          ...
          expect(winSum + lossSum, closeTo(1.0, 1e-10), reason: 'All-in probabilities should sum to 1.0 for ($a, $d)');
